...
handler = logging.FileHandler('./logs/5_fold_cross_validations_logs/multi_cnn3_all_conv_model_{}.log'.format(date_time))
...
csv_logger = CSVLogger('./logs/5_fold_cross_validations_logs/per_epoch_multi_cnn3_all_conv_model{}'.format(date_time), append=True, separator=';')
...
for fold in range(len(cv_folds_users)):
    ...
    model.fit([train_x, train_y, train_z], target_train, callbacks=[csv_logger], epochs=num_epochs)
    ...
    # Save model
    model.save('./models/5_fold_cross_validations_models/multi_cnn3_all_conv_model_{}_fold_{}'.format(date_time, fold))
